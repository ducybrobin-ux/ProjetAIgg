...
.\AIgg.cmd tests        # auto-diagnostics (289 vérifications réelles)
```
...
- **Cognition — orchestration cognitive (socle)** : la fin du « je ne sais pas » sec —
  AIgg passe par un cycle **réel** `rappel mémoire → recherche bibliothèque →
  diagnostic du manque → plan + outils candidats` et annonce un **état cognitif
  honnête** (`JE_SAIS`, `J_AI_TROUVE`, `JE_PEUX_CHERCHER`,
  `JE_N_AI_PAS_OUTIL_PERMISSION`, `J_AI_BESOIN_DE_PRECISION`…). Rappel mémoire
  **jamais inventé** ; consultation des bibliothèques (L2) avec provenance ;
  **capacité ≠ permission** (outils candidats vérifiés installé+autorisé, **aucun
  outil exécuté** en socle) ; apprentissage ouvert → **besoin `QUESTION`** au tuteur,
  réponse mémorisée et besoin clôturé.
- **Conversation enrichie** : « Je ne sais pas encore répondre à cela (…) — État
  cognitif : … » ; `/api/talk` expose le résultat cognitif ; CLI
  `AIgg.cmd cognition "question"`.
- La recherche OUTIL réelle (Web multi-sources, IA externe comme outil) arrive en
  **v0.5.1**.
